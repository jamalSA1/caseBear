...
export default function DesignConfiguratord({
  configId,
  imageUrl,
  imageDimensions
      ...
      <div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
        <ScrollArea className="relative flex-1 overflow-auto">
          ...
          <div className="px-8 pb-12 pt-8">
            ...
            <div className="w-full h-px bg-zinc-200 my6" />
                      ...
                      <Radio
                        ...
                        className={({ checked }) =>
                          cn(
                            ...
                            {
                              'border-zinc-900': checked && color.tw === 'zinc-900',
                              'border-blue-950': checked && color.tw === 'blue-950',
                              'border-rose-950': checked && color.tw === "rose-950",
                            }
                          )
                        }
                      >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" role="combobox" className="w-full justify-between">{options.model.label}
                  <ChevronDownIcon className="ml-2 w-4 h-4 shrink-0 opacity-50" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
                {MODELS.options.map((model) => (
                  <DropdownMenuItem key={model.label} className={cn("flex gap-1 items-center text-sm cursor-default hover:bg-zinc-100", {
              ...
              onChange={(val) => 
                ...
                <div className="mt-3 space-y-4">
                  {selectableOptions.map((option) => (
                    <RadioGroup.Option key={option.label} value={option} className={({active, checked}) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between", {
                      ...
                      <span className="flex items-center">
                        <span className=" flex flex-col text-sm">
                          ...
                          {option.description ? (
                            <RadioGroup.Description className="text-sm text-gray-500" as='span'>
